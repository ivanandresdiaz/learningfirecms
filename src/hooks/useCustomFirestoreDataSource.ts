...
import { resolveCollection, sanitizeData, updateDateAutoValues } from "firecms";
import {
 ...
 * @category Firebase
 */
export function useCustomFirestoreDataSource(
  params: // databaseId,
  FirestoreDataSourceProps
): DataSource {
  const { firebaseApp, textSearchController, fields } = params;
  // const databaseId = params.databaseId;
  // const databaseId = "(default)";
  const [databaseId] = useLocalStorage(
    "databaseId",
    // "extrabase"
    "(default)"
  ); // To work with Multiple database
  // console.log("databaseId", databaseId);
  // console.log("CUSTOM DATA SOURCEdatabaseId", databaseId);
  ...
  const performTextSearch = useCallback(
    async <M extends Record<string, any>>(
      ...
      return Promise.all(promises).then(
        (res) => res.filter((e) => e !== undefined && e.values) as Entity<M>[]
      );
    },
    [getAndBuildEntity, databaseId]
  );
  ...
  return {
    ...
    fetchCollection: useCallback(
      ...
      [buildQuery, performTextSearch, databaseId]
    ),
      ...
      <M extends Record<string, any>>({
        ...
        return onSnapshot(query, {
          next: (snapshot) => {
            onUpdate(
              snapshot.docs.map((doc) =>
                createEntityFromCollection(doc, path, resolvedCollection)
              )
            );
          },
          error: onError,
      ...
      <M extends Record<string, any>>({
        ...
        return onSnapshot(doc(firestore, path, entityId), {
          next: (docSnapshot) => {
            ...
            onUpdate(
              createEntityFromCollection(docSnapshot, path, resolvedCollection)
            );
          },
          error: onError,
        });
    ...
    saveEntity: useCallback(
      <M extends Record<string, any>>({
        ...
        const collectionReference: CollectionReference = collectionClause(
          firestore,
          path
        );
        ...
        const updatedFirestoreValues: EntityValues<M> = updateDateAutoValues({
          ...
          setDateToMidnight: (input?: Timestamp): Timestamp | undefined => {
        ...
        if (entityId) documentReference = doc(collectionReference, entityId);
