 ...
 * @category Firebase
 */
export function useInitialiseCustomFirebase({
  firebaseConfig,
  onFirebaseInit,
  name,
  authDomain,
  ...
  >();
  const [firebaseConfigLoading, setFirebaseConfigLoading] =
  ...
  const initFirebase = useCallback(
    (config: Record<string, unknown>) => {
      try {
        const initialisedFirebaseApp = initializeApp(
          config,
          name ?? "[DEFAULT]"
        );
        setConfigError(undefined);
        setFirebaseConfigLoading(false);
        if (onFirebaseInit) onFirebaseInit(config);
